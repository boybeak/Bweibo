 ...
 * @since 2013-11-22
 */
public class Status implements Serializable, SummaryFragment.SummaryProvider {
    ...
    public ArrayList<Thumbnail> pic_urls;
    /** 微博流内的推广微博ID */
    //public Ad ad;
    ...
    public static Status parse(JSONObject jsonObject) {
        ...
        if (picUrlsArray != null && picUrlsArray.length() > 0) {
            ...
            status.pic_urls = new ArrayList<Thumbnail>(length);
            JSONObject tmpObject = null;
            for (int ix = 0; ix < length; ix++) {
                tmpObject = picUrlsArray.optJSONObject(ix);
                if (tmpObject != null) {
                    //TODO ??? delete this kind of static method??
                    //status.pic_urls.add(tmpObject.opttmpObject.optString("thumbnail_pic"));
                }
            }
        }
    ...
    public boolean hasRetweetOne () {
        return retweeted_status != null;
    }

    @Override
    public CharSequence getTitle() {
        return PatternUtils.translate("@" + user.name + ":" + text);
    }

    @Override
    public CharSequence getSubTitle() {
        if (retweeted_status != null) {
            if (retweeted_status.user != null) {
                return PatternUtils.translate("@" + retweeted_status.user.name + ":" + retweeted_status.text);
            } else {
                return PatternUtils.translate(retweeted_status.text);
            }
        }
        return null;
    }
